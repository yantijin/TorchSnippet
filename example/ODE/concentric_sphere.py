...
if name == 'augment':
    ...
    class aModel(nn.Module):
        def __init__(self, augment_dim=1, out_dim=2):
            ...
            self.basis_mdoel = tsp.dyna.NeuralODE(func, t=torch.linspace(0, 1, 2).to(device), last=True).to(device)
    ...
    class aModel(nn.Module):
        def __init__(self, augment_dim=1, out_dim=2):
            super(aModel, self).__init__()
            self.augment_Dim = augment_dim
            self.augment = tsp.dyna.Augmenter(augment_dims=self.augment_Dim, order='second')
    ...
    class aModel(nn.Module):
        def __init__(self, augment_dim=1, out_dim=2):
            super(aModel, self).__init__()
            self.augment_Dim = augment_dim
            self.augment = tsp.dyna.Augmenter(augment_dims=self.augment_Dim, order='second', # augment_func=nn.Linear(2, 1).to(device),
                                              augment_idx=-1)# note: this is used for plot
            # func = ODEFunc(augment_dim=self.augment_Dim)
            func = nn.Sequential(nn.Linear(4, 50), nn.Tanh(), nn.Linear(50, 2))
            self.basis_mdoel = tsp.dyna.NeuralODE(func, t=torch.linspace(0, 1, 2).to(device), last=True, order=2).to(device)
            # self.linear = nn.Linear(self.basis_mdoel.func.in_dim+self.augment_Dim, out_dim)
