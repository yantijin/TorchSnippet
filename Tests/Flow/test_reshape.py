...
from TorchSnippet.Flow.reshape import *
from ..helper import *
from ..ops import *
...
class TestSpaceDepthTransformFlow():
    ...
    def test_space_depth_transform(self):
        ...
        for spatial_ndims, batch_shape, block_size in product(
            ...
            y = getattr(tsp.Flow, f'space_to_depth{spatial_ndims}d')(x, block_size)
            log_det = torch.zeros(batch_shape)
            input_log_det = torch.randn(batch_shape)

            # construct the classes
            cls = getattr(tsp.Flow, f'SpaceToDepth{spatial_ndims}d')
            inv_cls = getattr(tsp.Flow, f'DepthToSpace{spatial_ndims}d')
