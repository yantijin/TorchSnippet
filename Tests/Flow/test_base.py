...
class TestInverseFlow(object):
    ...
    def test_InverseFlow(self):
        ...
        with pytest.raises(TypeError,
            ...
            _ = InverseFlow(tsp.Layers.Linear(5, 3))
...
class TestSequentialFlow(object):
    ...
    def test_constructor(self):
        ...
        with pytest.raises(TypeError,
            ...
            _ = SequentialFlow([tsp.Layers.Linear(5, 3), _MyFlow()])

        with pytest.raises(TypeError,
                           match=r'`flows\[1\]` is not a flow'):
            _ = SequentialFlow([_MyFlow(), tsp.layers.Linear(5, 3)])
    ...
    def test_invertible_matrices(self):
        for cls in (LooseInvertibleMatrix, StrictInvertibleMatrix):
            for n in [1, 3, 5]:
                ...
                # check the gradient
                matrix, log_det = m(inverse=False, compute_log_det=True)
                params = [v for _, v in tsp.core.get_parameters(m)]
                grads = list(torch.autograd.grad([tsp.core.reduce_sum(matrix), tsp.core.reduce_sum(log_det)], params))
