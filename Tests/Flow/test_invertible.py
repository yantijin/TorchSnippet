...
from TorchSnippet.Flow.invertible import *
from Tests.helper import flow_standard_check
from TorchSnippet.core import flatten_to_ndims, unflatten_from_ndims, reduce_sum
    ...
    def test_invertible_dense(self):
        ...
        for strict in (True, False):
            check_invertible_linear(
                ...
                linear_factory=tsp.Layers.Linear,
                strict=strict
            )
            ...
            for strict in (True,):
                ...
                check_invertible_linear(
                    ...
                    invertible_linear_factory=getattr(
                        tsp.Flow, f'InvertibleConv{spatial_ndims}d'),
                    linear_factory=getattr(
                        tsp.Layers, f'LinearConv{spatial_ndims}d'),
                    strict=strict
                )
