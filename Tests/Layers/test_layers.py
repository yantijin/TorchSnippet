...
def check_composed_layer(ctx, input, layer_cls, linear_cls, normalizer_cls,
    ...
    # test activation
    activation_cls = tsp.Layers.Tanh
    for activation_arg in [activation_cls, activation_cls()]:
        ...
        assert isinstance(layer[0], linear_cls)
        assert isinstance(layer[1], tsp.Layers.Tanh)
        assert(
    ...
    activation = tsp.Layers.LeakyReLU()
    layer = layer_cls(
    ...
    def test_conv_nd(self):
        for spatial_ndims in (1, 2, 3):
            check_composed_layer(
                ...
                getattr(tsp.Layers, f'Conv{spatial_ndims}d'),
                getattr(tsp.Layers, f'LinearConv{spatial_ndims}d'),
                getattr(tsp.Layers, f'BatchNorm{spatial_ndims}d'),
                4, 3,
        ...
        for spatial_ndims in (1, 2, 3):
            for output_padding in (0, 1):
                check_composed_layer(
                    ...
                    getattr(tsp.Layers, f'ConvTranspose{spatial_ndims}d'),
                    getattr(tsp.Layers, f'LinearConvTranspose{spatial_ndims}d'),
                    getattr(tsp.Layers, f'BatchNorm{spatial_ndims}d'),
                    4, 3,
                    kernel_size=3, stride=2, dilation=2, padding='half',
