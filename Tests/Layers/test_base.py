import pytest
from TorchSnippet.Layers.base import *
from TorchSnippet.Layers import initializer
from TorchSnippet.core import *
import numpy as np
import torch
from TorchSnippet.core import as_tensor
...
class TestCoreLinearCase(object):
    ...
    def test_conv_nd(self):
        def do_check(spatial_ndims, kernel_size, stride, dilation, padding):
            ...
            layer_factory = getattr(tsp.Layers, cls_name)
            check_core_linear(
                self,
                np.random.randn(
                    *make_conv_shape(
...
class TestBatchNorm(object):
    def test_batch_norm(self):
        ...
        for spatial_ndims in (0, 1, 2, 3):
            cls = getattr(tsp.Layers, ('BatchNorm' if not spatial_ndims
...
class TestDropout(object):
    def test_dropout(self):
        n_samples = 10000
        for spatial_ndims in (0, 1, 2, 3):
            cls = getattr(tsp.Layers, ('Dropout' if not spatial_ndims
