...
from TorchSnippet.core import Tensor
...
def ensure_stacks_causality(ctx,
    ...
    for i in range(spatial_ndims):
        output = outputs[i]
        if isinstance(output, tsp.Tensor):
            output = tsp.to_numpy(output)
    ...
    def test_causality_and_receptive_field(self):
        for size in [[12], [12, 11], [12, 11, 10]]:
            ...
            for kernel_size in [3, 5, [5, 3, 5][:spatial_ndims]]:
                ...
                # resblock_layer = tsp.layers.jit_compile(resblock_layer)

                with pytest.raises(Exception):
                    _ = resblock_layer([tsp.zeros([])] * (spatial_ndims - 1))
                with pytest.raises(Exception):
                    _ = resblock_layer([tsp.zeros([])] * (spatial_ndims + 1))

                # the down-sampling and up-sampling layer
                down_sample_cls = getattr(tsp.Layers, f'PixelCNNConv{spatial_ndims}d')
                down_sample_layer = down_sample_cls(1, 1, kernel_size, stride=2)
                # down_sample_layer = tsp.Layers.jit_compile(down_sample_layer)

                down_sample_output_size = list(down_sample_layer(
                    [tsp.zeros(make_conv_shape([1], 1, size))] * spatial_ndims)[0].shape)
                up_sample_cls = getattr(tsp.Layers, f'PixelCNNConvTranspose{spatial_ndims}d')
                up_sample_layer = up_sample_cls(
                    1, 1, kernel_size, stride=2,
                    output_padding=tsp.Layers.get_deconv_output_padding(
                ...
                output_layer_cls = getattr(
                    tsp.Layers, f'PixelCNNOutput{spatial_ndims}d')
                output_layer = output_layer_cls()
                # output_layer = tsp.layers.jit_compile(output_layer)

                with pytest.raises(Exception,
                                   match=r'`len\(inputs\)` is expected to be .*'):
                    _ = output_layer([tsp.zeros([])] * (spatial_ndims - 1))
                with pytest.raises(Exception,
                                   match=r'`len\(inputs\)` is expected to be .*'):
                    _ = output_layer([tsp.zeros([])] * (spatial_ndims + 1))
                ...
                x_t = tsp.zeros(make_conv_shape([1], 1, size), dtype=torch.float32)
                outputs = input_layer(x_t)
                assert(
                    (np.abs(tsp.to_numpy(outputs[-1])) >= 1e-6).astype(np.int32) ==
                    x_t.cpu().numpy()
                ).all()
    ...
    def test_pixelcnn_network(self):
        ...
        for size in [[15], [15, 13], [15, 13, 11]]:
            ...
            for kernel_size in [3, 5, [5, 3, 5][:spatial_ndims]]:
                ...
                # input_layer = tsp.Layers.jit_compile(input_layer)

                # the pixelcnn layers
                resblock_layer_cls = getattr(
                    tsp.Layers, f'PixelCNNResBlock{spatial_ndims}d')
                conv_layer_cls = getattr(
                    tsp.Layers, f'PixelCNNConv{spatial_ndims}d')
                deconv_layer_cls = getattr(
                    tsp.Layers, f'PixelCNNConvTranspose{spatial_ndims}d')
                normalizer_cls = getattr(
                    tsp.Layers, f'BatchNorm{spatial_ndims}d')
                dropout_cls = getattr(
                    tsp.Layers, f'Dropout{spatial_ndims}d')

                pixelcnn_layers = [
                    resblock_layer_cls(
                        out_channels, out_channels, kernel_size=kernel_size,
                        activation=tsp.Layers.LeakyReLU, normalizer=normalizer_cls,
                        merge_context1=_MyAddContext,
                        data_init=tsp.Layers.StdDataInit,
                    ),
                    conv_layer_cls(
                        out_channels, out_channels, kernel_size=kernel_size,
                        stride=2, activation=tsp.Layers.Tanh, normalizer=normalizer_cls,
                        data_init=tsp.Layers.StdDataInit,
                    ),
                    deconv_layer_cls(
                        out_channels, out_channels, kernel_size=kernel_size,
                    ...
                    resblock_layer_cls(
                        out_channels, out_channels, kernel_size=kernel_size,
                        activation=tsp.Layers.Sigmoid, normalizer=normalizer_cls,
                ...
                network_cls = getattr(tsp.Layers, f'PixelCNN{spatial_ndims}d')

                with pytest.raises(TypeError,
                                   match='`input_layer` must be an instance of'):
                    _ = network_cls(tsp.Layers.Linear(2, 3))
                ...
                tsp.set_train_mode(network1, False)
                tsp.set_train_mode(network2, False)
